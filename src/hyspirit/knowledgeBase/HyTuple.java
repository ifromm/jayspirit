 ...
 * queries. Each tuple has a certain probability, which is a value between 0
 * and 1.
 *  
...
public class HyTuple implements Comparable<HyTuple> {
     ...
     * @param probability the tuple probability
     * @param attributeValues the attribute values of the tuple
     */
    ...
    /**
     * Constructor of class. This constructor takes a line as returned by
     * an inference engine of the form
     * <br>
     * &lt;prob&gt; (&lt;tokenelement_1&gt;,...,&lt;tokenelement_n&gt;)
     * <br>
     * parses it and creates a corresponding HyTuple element
     * 
     * @param line the line as returned by the inference engine
     * @throws HyTupleFormatException if the input line does not represent a
     *         tuple
     */
    public HyTuple(String line) throws HyTupleFormatException {
        ...
        if (m.find()) {
            ...
            if (m.find()) this.probability = 1;
            else throw 
                new HyTupleFormatException("Malformed probability in tuple!");
        }
        ...
        if (m.find()) {
            String attributeString =
                m.group().substring(1, m.group().length()-1); // remove brackets
            
            m = Pattern.compile("^(\".*?\"|[^,\"]*),?").matcher(attributeString);
            Vector<String> attValues = new Vector<String>();
            while (m.find() && ! attributeString.equals("")) {
                String attributeValue = m.group();
                attributeString =
                    attributeString.substring(m.end(), attributeString.length())
                                                                    .trim();
        ...
        else throw new HyTupleFormatException("Malformed attributes in tuple!");
    ...
    /**
     * Returns a string representation of the tuple, i.e.,
     * <br>
     * &lt;prob&gt; (&lt;tokenelement_1&gt;,...,&lt;tokenelement_n&gt;)
     * 
     ...
     * @param tupleObj the tuple this tuple is compared with
     * @return 1, if this object has a lower probability than the other, -1, 
     *         if this object has a higher probability, 0 if the probability is
     *         the same
     */
    @Override
	public int compareTo(HyTuple tuple) {
        if (tuple.probability() > this.probability()) return 1;
        else if (tuple.probability() < this.probability()) return -1;
        else return 0;
    }
