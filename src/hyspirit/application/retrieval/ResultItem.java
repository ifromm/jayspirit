...
public class ResultItem implements Comparable<ResultItem> {
     ...
     * @param rsv the retrieval status value of the item
     * @param uri the uniform resource identifier of the item 
     */
     ...
     * probability.
     * @return an English string representation of the 
     ...
     * @param rItem the result item this object is compared with
	...
	public int compareTo(ResultItem rItem) {
        ResultItem r = rItem;
        if (r.getRSV() > this.getRSV()) return 1;
        else if (r.getRSV() < this.getRSV()) return -1;
        else return 0;
    }
}
