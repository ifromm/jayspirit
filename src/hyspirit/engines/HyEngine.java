...
public abstract class HyEngine implements Runnable {
    ...
    throws HySpiritException {
        if (hyspirit == null) hyspirit = new HySpiritProperties();
...
//        processThread.start();
    ...
    /**
     * Returns the absolute path of the current command if in client/server
     * mode and null if in client mode
     */
    ...
    public BufferedWriter getOutputWriter() throws IOException{
        if (clientmode) return getSocketIn();
        else return getSTDIN();
    }
    ...
    public BufferedReader getInputReader() throws IOException{
        if (clientmode) return getSocketOut();
        else return getSTDOUT();
    }
    ...
    @Override
	public void run(){
        if (!clientmode) run(buildCommand(), hyspirit);
    }
...
*/
     ...
     * @throws IllegalThreadStateException if the process hasn't started yet.
     */
    public void destroy() throws IllegalThreadStateException {
        if (!clientmode) {
            ...
            else throw new IllegalThreadStateException("Process not started!");
        }
    }
     ...
     * has terminated. By convention, 0 indicates normal termination. If in
     * client mode, this does nothing and returns -1.
    ...
    public boolean isRunning() {
        if (verbose) System.out.println(getEngineName() + ".isRunning: process = " + process);
        if (clientmode || process != null) return true;
        else return false;
    }
    ...
    /**
     * This does nothing but wait until an underlying bridge process is
     * actually running, and it return then. Do not invoke if you did not
     * invoke the run() or start() method first! Returns immediately in client
     * mode
     *
     */
    public void waitTillRunning() {
        if (!clientmode) while (! isRunning()) {}
    }
     ...
     * @param input the string to be sent to the engine.
     */
     ...
     * process. Be sure you read the results of a previous send() first because
     * otherwise they will be dropped!
     * @param filename the content of this file will be sent to the engine
     */
    ...
    protected void send(BufferedReader input) throws IOException{
        ...
        else throw new IOException("in, input or out null!");
    }
     ...
     * System.err and to the engine's logger. If you do not want this, you
     * can suppress this here. The error output of the process is still captured
     * by the Logger on the WARN level.
     * @param suppress true if the process output should not be sent to 
     *         System.err, false else.
     */
     ...
     * command with the format defined in {@link #TIME_FORMAT}. The output
     * is appended to STDOUT.
     * @param takeTime whether to take file or not. Default: don't take time
     *  {@code false}.
     */
    ...
    /**
     * Set the knowledge base of this engine.
     * This method creates a knowledge base object with the argument name.
