 ...
 * running in their own thread. Reading if the input stream is finished when
 * a delimiter is read or the end of stream is reached.
 * 
 * @author Ingo Frommholz &lt;ingo@is.informatik.uni-duisburg.de&gt;
 * <p>
 * Created on 12-Jun-2005 20:22:25. Imported to JaySpirit 02-Dec-2005.
...
public class StreamCatcher extends Thread {
	...
	Logger LOG = Logger.getLogger(StreamCatcher.class);

	/**
	 * Constructor of class
	 * @param in  a buffered reader, e.g. the STDOUT or STDERR from another
	 * 		  executable.
	 * @param delimiter the string nofifying this thread to stop reading
	 */
	 ...
	 * @param verbose true if verbose mode should be set, false otherwise
	 */
	...
	/**
	 * Gets the next line of the stream or null if the stream is finished. If
	 * we did not receive a delimiter yet but already delivered the last recent
		...
		if (in != null) {
			...
			try {
				while ((line = in.readLine()) != null) {
					LOG.trace(line);
					//Changed how the end of line dilimiter was handled.  The entire line does not necessarily need to match the delimiter.
					if (true) {
