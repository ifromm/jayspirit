...
public class DocFreqList {
    // Enable logging
    private static Logger LOG = LogManager.getLogger(DocFreqList.class);

    /**
     * Whether to use max_idf norm for some calculations (e.g. when using
     * {@link #writeList(BufferedWriter)})
     */
    public final static int MAX_IDF = 0;

    /**
     * Whether to use sum_idf norm for some calculations (e.g. when using
     * {@link #writeList(BufferedWriter)})
     */
    public final static int SUM_IDF = 1;

    private final HashMap<String, Integer> termIDF = new HashMap<String, Integer>();
     ...
     */
    private final HashSet<String> termdocs = new HashSet<String>();

    /**
     * Keeps track of documents
     */
    private final HashSet<String> docs = new HashSet<String>();
     ...
     * pairs with the {@link #addTermDoc(String, String)} method, this input has
     * to be sorted in two ways:
     * <ul>
     ...
     * document is submitted, there must not be any term-document pair with
     * another already submitted document.
     * </ul>
     ...
     * @param idfNorm
     *            which norm to use ({@link #MAX_IDF} or {@link #SUM_IDF})
     * @throws IOException
